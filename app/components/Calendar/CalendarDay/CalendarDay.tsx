...
export const CalendarDay = (props: CalendarDayProps) => (
    <div
        key={`${props.month}-${props.day}`}
        ref={props.dayRefCallback}
        data-month={props.month}
        data-day={props.day}
        onClick={() => props.onClick(props.day, props.month, props.year)}
    ...
    >
        <span className={`absolute left-1 top-1 flex size-5 items-center justify-center rounded-full text-xs sm:size-6 sm:text-sm lg:left-2 lg:top-2 lg:size-8 lg:text-base ${props.isToday ? 'bg-blue-500 font-semibold text-white' : ''} ${props.month < 0 ? 'text-slate-400' : 'text-slate-800'}`}>
            {props.day}
        </span>
        ...
        {props.events.length > 0 && (
            <div className="absolute top-7 left-1 right-1 flex flex-col gap-1 z-20 sm:top-8 lg:top-10">
                {props.events.slice(0, 3).map((e, idx) => (
                    <div
                        key={idx}
                ...
                {props.events.length > 3 && (
                    <span className="text-[10px] text-gray-400 font-medium">+{props.events.length - 3} autres</span>
                )}
            </div>
        )}
        ...
        <button
            ...
            onClick={(e) => {
                e.stopPropagation();
                props.onQuickAdd(props.day, props.month, props.year);
            }}
